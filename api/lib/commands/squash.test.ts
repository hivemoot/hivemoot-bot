...
describe("/squash command", () => {
  ...
  it("refreshes when mergeable_state resolves from unknown to behind", async () => {
    vi.useFakeTimers();
    const octokit = createMockOctokit();
    let pullsGetCalls = 0;
    octokit.rest.pulls.get.mockImplementation(async () => {
  ...
  it("blocks merge when mergeable state never resolves", async () => {
    vi.useFakeTimers();
    const octokit = createMockOctokit();
    ...
    expect(octokit.rest.pulls.merge).not.toHaveBeenCalled();
    const body = (octokit.rest.issues.createComment.mock.calls[0][0] as { body: string }).body;
    expect(body).toContain("GitHub has not finished computing mergeability");
    expect(body).toContain("stale head");
  });
