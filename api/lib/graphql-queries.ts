...
async function getLinkedIssuesFallbackFromBody(
  ...
  if (failed > 0 && linkedIssues.length === 0) {
    throw new Error(
      `Failed to resolve referenced issues from PR body fallback for ${owner}/${repo}#${prNumber}`
    );
  }
...
export async function getLinkedIssues(
  ...
  try {
    ...
    logger.warn(
      `closingIssuesReferences is unavailable for ${owner}/${repo}#${prNumber}; falling back to PR body parsing`
    );
