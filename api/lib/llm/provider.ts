...
function createModelWithApiKey(config: LLMConfig, apiKey: string): LanguageModel {
  switch (config.provider) {
    ...
    case "google": {
      ...
      // For Gemini, structuredOutputs must be disabled — see GOOGLE_STRUCTURED_OUTPUTS_FALSE
      // helper below. In ai SDK v6, model-level options are passed via providerOptions
      // in the generateObject() call, not at model construction time.
...
/**
 * Build provider-specific options for generateObject() calls.
 *
 * Gemini: structuredOutputs must be false — native responseSchema serialization
 * breaks newer Gemini models (gemini-3-*). In ai SDK v6 these options are passed
 * at the call site via providerOptions, not at model construction time.
 *
 * References:
 *   • AI SDK docs: https://ai-sdk.dev/providers/ai-sdk-providers/google-generative-ai#structured-outputs
 *   • SDK source: responseSchema is populated only when supportsStructuredOutputs is true
 */
export function providerOptions(provider: LLMProvider): Record<string, Record<string, boolean | string | number | null>> | undefined {
  if (provider === "google") {
