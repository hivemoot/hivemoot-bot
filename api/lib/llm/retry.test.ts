...
describe("withLLMRetry", () => {
  ...
  it("allows retry when delay fits within maxTotalElapsedMs budget", async () => {
    ...
    const log = mockLogger();
    // Budget of 10s, delay is 2s — should allow retry
  ...
  it("aborts mid-sequence when cumulative time exhausts budget", async () => {
    ...
    // Three rate-limit failures. With 50ms delay each, total = 150ms.
    // Budget of 100ms should allow first retry (50ms elapsed + 50ms delay = ok)
    // but abort before second retry (100ms elapsed + 50ms delay > 100ms).
    const fn = vi
      .fn()
      .mockRejectedValueOnce(rateLimitError)
      .mockRejectedValueOnce(rateLimitError)
