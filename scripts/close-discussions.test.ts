...
describe("close-discussions script", () => {
  ...
  describe("processIssuePhase", () => {
    ...
    it("should report rate-limited issues via onAccessIssue", async () => {
      // 429 is transient and retried by withRetry; after maxAttempts it is
      // caught by processIssuePhase's outer handler and reported as rate_limit
      mockGetLabelAddedTime.mockRejectedValue(
      ...
      // Advance fake timers so withRetry's backoff delays fire
    ...
    it("should rethrow unexpected errors after retries", async () => {
      // 500 is transient and retried by withRetry; after maxAttempts it is rethrown
      mockGetLabelAddedTime.mockRejectedValue(
  ...
  describe("isTransientError (from shared module)", () => {
